...
- 4차의 endpoint-only Color 패턴에서는 WCS `LOGCOLORSPACE`의 단일 power gamma 한계 때문에 BT.2020에 `2.4` 근사를 사용했다. 7차부터는 색역별 inverse transfer function으로 입력을 먼저 선형화하고 gamma `1.0` source로 WCS에 전달한다.
...
| `TestColors.h` | 색역, 전달 함수, 테스트 패턴과 patch sequence 정의 |
| `TestRenderer.h/.cpp` | 색역별 렌더러 인터페이스와 생성 |
| `GdiSrgbRenderer.h/.cpp` | 기존 GDI sRGB 출력 |
| `ColorManagedRenderer.h/.cpp` | Display-P3, Adobe RGB 및 BT.2020의 선형화와 모니터별 Advanced Color 또는 ICC/WCS 출력 |
